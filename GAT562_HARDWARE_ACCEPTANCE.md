...
| Fixed menu | Single left-side rolling menu rail | 24x24 fixed-menu C++ bitmap icons; two visible at a time; triangle arrow marks the current selection; right rail removed so the main scene is 96x64 | CODED, BUILT, FLASH BLOCKED, HW OPEN | Simulator proofs: `analysis\screen_simulator\out\compact_home_x4.png` and `analysis\screen_simulator\out\fixed_menu_icons_proof_x2.png`. Needs GAT562 replug/reset, flash, then OLED confirmation. |
...
| Toilet cleanup animation | Connection cleanup-wall contract | Right-side vertical checker clear-wall travels right-to-left and erases pet plus poop from right to left; pet and poop stay in their scene positions until clipped by the wall; no broom, water wave, eaten-poop motion, heart finish, or center text | SPEC LOCKED, CODED, SIM-PREVIEWED, FLASH BLOCKED, HW OPEN | Compact simulator proof: `analysis\screen_simulator\out\compact_toilet_cleanup_wall_contact_x4.png`. The scene now snapshots pre-cleanup `messCount` so poop remains visible until the wall clips it away, and the latest fix keeps pet/poop stationary while only the wall moves. Needs GAT562 replug/reset, flash, then OLED confirmation. |
...
| `GAT562-DISPLAY-001A` | Cycle through all ten fixed-menu actions on the 128x64 OLED | Only the left rolling menu rail is shown; the current selection has a triangle arrow; each icon remains a readable 24x24 Tamagotchi-style tile; the first five semantics are gauge, chef-face, toilet, baseball/bat, signal-heart | HW OPEN | Awaiting user observation after flash. |
...
| `GAT562-DISPLAY-003` | Trigger Toilet cleanup while poop is present | A right-side vertical checker clear-wall travels right-to-left; pet and poop do not slide toward each other, and each is clipped/erased only as the wall reaches it; no broom, water wave, eaten-poop motion, heart finish, or center text appears; after the animation clears, the UI returns to the home playfield instead of showing the no-mess page | HW OPEN | Awaiting user observation. |
